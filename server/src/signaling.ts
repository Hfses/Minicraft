...
export class SignalingHub {
  ...
  private onClientMessage(socket: WebSocket, msg: SignalClientMessage): void {
    switch (msg.type) {
      ...
      case "kick":
        ...
        break;
      case "ping": {
        // A live signaling connection means the room is in active use, even
        // when nobody has chatted recently — count it as activity so
        // sweepExpired() never closes a room the players are still sitting in.
        const conn = this.connFor(socket);
        if (conn) {
          const room = this.store.getRoomById(conn.roomId);
          if (room) this.store.touch(room);
        }
  ...
  private onKick(socket: WebSocket, peerId: string): void {
    ...
    this.bySocket.delete(target.socket);
  ...
  private register(conn: Conn): void {
    ...
    this.bySocket.set(conn.socket, conn);
